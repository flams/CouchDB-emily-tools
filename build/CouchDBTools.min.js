...
define("CouchDBStore",["Store","StateMachine","Tools","Promise"],function(j,f,i,g){function c(){var b=null,a={},c=new g,d={getView:function(){a.query=a.query||{};b.request("CouchDB",{method:"GET",path:"/"+a.database+"/_design/"+a.design+"/"+a.view,query:a.query},function(b){var k=JSON.parse(b);if(k.rows)this.reset(k.rows),c.fulfill(this),typeof k.total_rows=="undefined"&&this.setReducedViewInfo(true),e.event("subscribeToViewChanges");else throw Error("CouchDBStore ["+a.database+", "+a.design+", "+
a.view+"].sync() failed: "+b);},this)},getDocument:function(){b.request("CouchDB",{method:"GET",path:"/"+a.database+"/"+a.document,query:a.query},function(a){var b=JSON.parse(a);b._id?(this.reset(b),c.fulfill(this),e.event("subscribeToDocumentChanges")):c.reject(a)},this)},getBulkDocuments:function(){var h={path:"/"+a.database+"/_all_docs",query:a.query},k;a.keys instanceof Array?(h.method="POST",h.data=JSON.stringify({keys:a.keys}),h.headers={"Content-Type":"application/json"},k=h.data):(h.method=
"GET",k=JSON.stringify(a.query));a.query.include_docs=true;b.request("CouchDB",h,function(b){var h=JSON.parse(b);if(h.rows)this.reset(h.rows),c.fulfill(this),e.event("subscribeToBulkChanges");else throw Error('CouchDBStore.sync("'+a.database+'", '+k+") failed: "+b);},this)},createDocument:function(h){b.request("CouchDB",{method:"PUT",path:"/"+a.database+"/"+a.document,headers:{"Content-Type":"application/json"},data:this.toJSON()},function(a){a=JSON.parse(a);a.ok?(h.fulfill(a),e.event("subscribeToDocumentChanges")):
h.reject(a)})},subscribeToViewChanges:function(){i.mixin({feed:"continuous",heartbeat:2E4,descending:true},a.query);this.stopListening=b.listen("CouchDB",{path:"/"+a.database+"/_changes",query:a.query},function(b){if(b=="\n")return false;var b=JSON.parse(b),c;c=a.reducedView?"updateReduced":b.deleted?"delete":b.changes&&b.changes[0].rev.search("1-")==0?"add":"change";e.event(c,b.id)},this)},subscribeToDocumentChanges:function(){this.stopListening=b.listen("CouchDB",{path:"/"+a.database+"/_changes",
query:{feed:"continuous",heartbeat:2E4,descending:true}},function(b){if(b=="\n")return false;b=JSON.parse(b);b.id==a.document&&b.changes.pop().rev!=this.get("_rev")&&(b.deleted?e.event("deleteDoc"):e.event("updateDoc"))},this)},subscribeToBulkChanges:function(){i.mixin({feed:"continuous",heartbeat:2E4,descending:true,include_docs:true},a.query);this.stopListening=b.listen("CouchDB",{path:"/"+a.database+"/_changes",query:a.query},function(a){if(a=="\n")return false;var a=JSON.parse(a),b;b=a.changes[0].rev.search("1-")==
0?"bulkAdd":a.deleted?"delete":"bulkChange";e.event(b,a.id,a.doc)},this)},updateDocInStore:function(c){b.request("CouchDB",{method:"GET",path:"/"+a.database+"/_design/"+a.design+"/"+a.view,query:a.query},function(a){a=JSON.parse(a);a.rows.length==this.getNbItems()?a.rows.some(function(a,b){a.id==c&&this.set(b,a)},this):this.actions.evenDocsInStore.call(this,a.rows,c)},this)},evenDocsInStore:function(a,b){var c=this.getNbItems();a.length<c?this.loop(function(a,c){a.id==b&&this.del(c)},this):a.length>
...
function(a){b.reject(a)})},this);return b}}return function(){i.prototype=new j;return new i}});define("CouchDBSecurity",["CouchDBStore"],function(j){function f(){var f="_security";this.setName=function(g){return g?(f=g,true):false};this.getName=function(){return f};this.load=function(g){return this.sync(g,f)}}return function(){f.prototype=new j;return new f}});
